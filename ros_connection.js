...
var startCmd = new ROSLIB.Topic({
    ...
    messageType : 'robot_config/Start'
});

var setVel = new ROSLIB.Topic({
    ros : ros,
    name: '/motor/set_velocity',
    messageType: 'robot_config/Track_Velocity'
})
...
var listenMotor = new ROSLIB.Topic({
    ros : ros,
    name : '/motor_status',
    messageType : 'robot_config/Track_Status'
});
...
var listenComments = new ROSLIB.Topic({
    ...
    messageType : 'robot_config/Executive_Comments'
});

// // Then we add a callback to be called every time a message is published on this topic.
listenComments.subscribe(function(message) {
console.log('Received message on ' + listenComments.name + ': ' + message.message);
document.getElementsByName('errors')[0].value = message.message;
